@extends('admin.layouts.master')
@section('title')
    File Downloads - {{ env('APP_NAME') }}
@endsection

@section('head')
    File Downloads Activity
@endsection

@section('content')
    <div class="main-content">
        <div class="inner_page">
            <!-- Statistics Cards -->
            <div class="row mb-4">
                <div class="col-lg-3 col-md-6">
                    <div class="mini_box small_bg_1">
                        <h3>{{ number_format($stats['total_downloads']) }}</h3>
                        <p>Total Downloads</p>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6">
                    <div class="mini_box small_bg_2">
                        <h3>{{ $stats['unique_users'] }}</h3>
                        <p>Unique Users</p>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6">
                    <div class="mini_box small_bg_3">
                        <h3>{{ $stats['today_downloads'] }}</h3>
                        <p>Today's Downloads</p>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6">
                    <div class="mini_box small_bg_4">
                        <h3>{{ $stats['this_week_downloads'] }}</h3>
                        <p>This Week</p>
                    </div>
                </div>
            </div>

            <!-- Filters -->
            <div class="card mb-4">
                <div class="card-body">
                    <form method="GET" class="row g-3">
                        <div class="col-md-3">
                            <select name="user_id" class="form-select">
                                <option value="">All Users</option>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ request('user_id') == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3">
                            <select name="course_id" class="form-select">
                                <option value="">All Courses</option>
                                @foreach ($courses as $course)
                                    <option value="{{ $course->id }}"
                                        {{ request('course_id') == $course->id ? 'selected' : '' }}>
                                        {{ $course->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2">
                            <input type="date" name="date_from" class="form-control" value="{{ request('date_from') }}"
                                placeholder="From Date">
                        </div>
                        <div class="col-md-2">
                            <input type="date" name="date_to" class="form-control" value="{{ request('date_to') }}"
                                placeholder="To Date">
                        </div>
                        <div class="col-md-2">
                            <button type="submit" class="btn btn-primary">Filter</button>
                            <a href="{{ route('user-activity.file-downloads') }}" class="btn btn-secondary">Reset</a>
                        </div>
                    </form>
                </div>
            </div>

            <!-- Downloads Table -->
            <div class="card table_sec">
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th>User</th>
                                <th>Course</th>
                                <th>File Name</th>
                                <th>IP Address</th>
                                <th>Downloaded At</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($downloads as $download)
                                <tr>
                                    <td>
                                        {{ $download->user->name }}<br>
                                        <small class="text-muted">{{ $download->user->email }}</small>
                                    </td>
                                    <td>{{ $download->course->name }}</td>
                                    <td>{{ $download->file_name }}</td>
                                    <td>{{ $download->ip_address }}</td>
                                    <td>{{ $download->downloaded_at->format('d M Y, h:i A') }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center">No downloads found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="d-flex justify-content-end mt-3">
                    {{ $downloads->appends(request()->query())->links() }}
                </div>
            </div>

            <!-- Top Downloaders -->
            <div class="card mt-4">
                <div class="card-header">
                    <h5>Top Downloaders</h5>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-sm">
                            <thead>
                                <tr>
                                    <th>User</th>
                                    <th>Downloads</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($topDownloaders as $downloader)
                                    <tr>
                                        <td>{{ $downloader->user->name }}</td>
                                        <td>{{ $downloader->download_count }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
